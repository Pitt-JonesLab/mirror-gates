...
"""Layout selection using the SABRE bidirectional search approach from Li et
al.
...
class SabreLayout(TransformationPass):
    """Choose a Layout via iterative bidirectional routing of the input
    circuit.
    ...
    **References:**
    ...
    def _compose_layouts(self, initial_layout, pass_final_layout, qregs):
        ...
        The routing passes internally start with a trivial layout, as
        the layout gets applied to the circuit prior to running them. So
        the "final_layout" they report must be amended to account for
